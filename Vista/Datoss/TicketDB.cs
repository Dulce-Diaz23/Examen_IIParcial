    ...
    public class TicketDB
    {
        ...
        public bool Guardar(Ticket ticket, List<DetalleTicket> detalle)
        {
            ...
            try
            {
                ...
                sqlTicket.Append("INSERT INTO ticket VALUES(Fecha, ISV, Descuento,Total, IdentidadCliente, CodigoUsuario) VALUES (@Fecha, @ISV, @Descuento,@Total, @IdentidadCliente, @CodigoUsuario);");
                sqlTicket.Append("SELECT LAST_INSERT_ID();");

                StringBuilder sqlDetalle = new StringBuilder();
                sqlDetalle.Append("INSERT INTO detalleticket VALUES_(Articulo, TipoSoporte, Solicitud, Respuesta,Cantidad, Precio,IdTicket, Total)  VALUES ( @Articulo, @TipoSoporte, @Solicitud, @Respuesta,@Cantidad, @Precio,@IdTicket, @Total);");
                ...
                using (MySqlConnection con = new MySqlConnection(cadena))
                {
                    ...
                    try
                    {
                        using (MySqlCommand cmd1 = new MySqlCommand(sqlTicket.ToString(), con, transaction))
                        {
                            ...
                            cmd1.Parameters.Add("@IdentidadCliente", MySqlDbType.Int32).Value = ticket.IdentidadCliente;
                            cmd1.Parameters.Add("@CodigoUsuario", MySqlDbType.Int32).Value = ticket.CodigoUsuario;
                        ...
                        foreach (DetalleTicket detalles in detalle)
                        {
                            using (MySqlCommand cmd2 = new MySqlCommand(sqlDetalle.ToString(), con, transaction))
                            {
                                ...
                                cmd2.Parameters.Add("@IdTicket", MySqlDbType.Int32).Value = detalles.IdTicket;
